<?php
//----security and path settings---------
$page_security = 'SA_HRM_DEPARTMENTS';
$path_to_root = "../..";
//----include files------------
include_once($path_to_root . "/includes/db_pager.inc");
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/includes/ui.inc");
include_once($path_to_root . "/includes/validation.inc");
include_once($path_to_root . "/includes/plusql.inc");
include_once($path_to_root . "/hrm/models/hrm_db.inc");
include_once($path_to_root . "/reporting/includes/reports_classes.inc"); 
//----page start-----------
simple_page_mode(true);
//----pager defined--------
$pager = "dept_tbl";
//----Database model defined--------
$department_db = new department_model();
$unit_db = new unit_model();
//----message controller----
$controller = "Department";
//---loggers----------------
pr("selected ID = $selected_id and Mode = $Mode");
pr($_POST);
//----form specific helper function	
function can_process()
{
	global $controller;
	return (!is_empty($_POST['strength'], 'Department strength')) &&  
			(!is_empty($_POST['unit_id'], 'Unit Name')) &&
			(!is_empty($_POST['name'], ' Department Name'))  &&
			(is_number($_POST['strength'], 'Department strength'));
}
//---add item----------------
if( (strcmp($Mode,'ADD_ITEM')==0) && can_process() )
{
	$flag = $department_db -> insert($_POST['name'],$_POST['strength'],$_POST['unit_id']);
 	if($flag == true)
 	{
 		add_msg($controller);
		$Mode = 'RESET';	
 	}
 	else
 	{
 		duplicate_msg($controller);
		set_focus('name');
 	}
}	
//---update item--------------
if( (strcmp($Mode,'UPDATE_ITEM')==0) && can_process() )
{
	$department_db -> update($selected_id, $_POST['name'],$_POST['strength'],$_POST['unitName']);
	update_msg($controller);
	$Mode = 'RESET';
}
//---delete item-------------
if(strcmp($Mode,'Delete')==0)
{
 	if (key_in_foreign_table($selected_id, 'employee', 'department_id'))
	{
		delete_error_msg($controller, 'Employee');
		$Mode = 'RESET';
	} 
	else 
	{ 
		$department_db -> delete($selected_id);
		delete_msg($controller);
		$Mode = 'RESET';
	 }
}
//---print item---------------
if(isset($_POST['PrintOrders']))
{ 
	$rep_file = find_custom_file("/hrm/reports/rep_".strtolower($controller).".php");
	if ($rep_file) 
	{
		require($rep_file);
	}
 	die();
}
//---reset the page------------
if(strcmp($Mode,'RESET')==0)
{
	refresh_pager($pager);
	$selected_id = -1; 
	unset($_POST); 
}
//---set the HTML----------	
page_start($controller."s");
//----start the new section of the page-----
new_headers_start($controller."s");
if ($selected_id != -1)
{
	if(strcmp($Mode,'Edit')==0)
	{
		$department_db -> select($selected_id,$_POST); 
	}
	hidden('selected_id', $selected_id);
} 
text_row_ex(_("$controller Name").':', 'name',50,50);
text_row_ex(_("$controller Strength").':', 'strength',10,10);
multiple_array_selector('unit_id', _("Unit Name"), $unit_db -> search(), 1);
new_headers_end($selected_id);	
//----start the search and print section of the page------------
search_headers_start($controller."s");
row_start();
ref_cells(_("$controller Name"), 'search_name', '',null, '', true);
check_cells("", "print_name", 1);
row_end();
row_start();
$arr = array('strength' => null);
multiple_array_selector('search_strength', _("$controller Strength"), $department_db -> search_advanced($arr));
check_cells("", "print_strength", 1);
row_end();
row_start();
$arr = array('name' => null);
multiple_array_selector('search_unit_id', _("Unit Name"), $unit_db -> search_advanced($arr), 1);
check_cells("", "print_unit_id", 1);
row_end();
search_headers_end(1,1,0);
//----start the pager section of the page---------
$th = array(_("$controller Name"),_("$controller Strength"),_("Unit Name"));
$sql = $department_db -> search(@$_POST['search_name'],@$_POST['search_strength'],@$_POST['search_unit_id']);
pager_display($pager,$th,$sql,"70%",$controller);
//----end the page---------
page_end();
?>
