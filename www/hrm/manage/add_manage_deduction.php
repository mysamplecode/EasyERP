...
$page_security = 'SA_HRM_DEDUCTIONS';
$path_to_root = "../..";
//----include files------------
...
function can_process()
{
	global $controller;
	return 
			(!is_empty($_POST['name'], ' Deduction Name'))  &&
			(!is_empty($_POST['description'], 'Deduction Description')) &&  
			(!is_empty($_POST['type'], 'Deduction Type'))
			;
}
//---add item----------------
if( (strcmp($Mode,'ADD_ITEM')==0) && can_process() )
{
	$flag = $deduction_db -> insert($_POST['name'],$_POST['description'],$_POST['type']);
 	if($flag == true)
 	{
 		add_msg($controller);
		$Mode = 'RESET';	
...
{
	$deduction_db -> update($selected_id, $_POST['name'],$_POST['description'],$_POST['type']);
...
{	//need to fill in later on
	...
	{ 
		$deduction_db -> delete($selected_id);
...
multiple_array_selector('type', _("Deduction Type"), $options, 0, 0);
...
row_start();
multiple_array_selector('search_type', _("Deduction Type"), $options, 0, 0);
...
$sql = $deduction_db -> search(@$_POST['search_name'],@$_POST['search_description'],@$_POST['search_type']);
...
{
	global $prefix;
	if(strcmp($row[$prefix.'deduction$type'],'0')==0)
