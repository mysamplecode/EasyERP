...
{
	$flag = $unit_db -> insert($_POST['name']);
 	if($flag == true)
 	{
 		add_msg($controller);
		$Mode = 'RESET';	
...
{
	$unit_db -> update($selected_id, $_POST['name']);
...
{
 	if (key_in_foreign_table($selected_id, 'department', 'unit_id'))
	{
		delete_error_msg($controller, 'Department');
		$Mode = 'RESET';
	} 
	else 
	{ 
		$unit_db -> delete($selected_id);
...
if ($selected_id != -1)
{
	...
	{
		$unit_db -> select($selected_id,&$_POST); 
	}
