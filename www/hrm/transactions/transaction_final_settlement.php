...
$page_security = 'SA_HRM_FINAL_SETTLEMENT_ENTRY';
$path_to_root = "../..";
//----include files------------
...
$transaction_code = EMPLOYEE_FINAL_SETTLEMENT;
//----message controller----
...
function can_process()
{
	...
	return 
			(!is_empty($_POST['transaction_reason'], ' Final Remarks'))  &&
			((strcmp($Mode,'UPDATE_ITEM')==0)?1:!is_empty($_POST['final_amount'], ' New Entry'))  &&
			(!is_empty($_POST['unit_id'], 'Unit')) &&  
			(!is_empty($_POST['department_id'], 'Department')) &&
			(!is_empty($_POST['employee_id'], 'Employee')) &&
			(is_number($_POST['final_amount'], 'Final Amount'))   
			;
}
//---add item----------------
...
{
	$epost = array();
	$employee_payroll_db -> select($employee_id,$epost);
	label_row("Basic Salary", $epost['basic_salary'],"","style = 'padding-left: 8px;'");
	label_row("Existing Installment", $exinstallment,"","style = 'padding-left: 8px;'");
	label_row("Existing Installment", $exinstallment,"","style = 'padding-left: 8px;'");
	label_row("Existing Installment", $exinstallment,"","style = 'padding-left: 8px;'");
	/*
	$spost = array();
	...
	{
		$exinstallment = $_POST['transaction_dec1'];
		$newinstallment = $_POST['transaction_dec2'];
		...
		row_end();
		label_row("New Installment", $newinstallment,"","style = 'padding-left: 8px;'");
	}
	else
	{
		...
		if(empty($exadvance))
		{
			label_row("Remaining Advance", '0.00',"","style = 'padding-left: 8px;'");
			hidden('remaining_advance','0.00');
		}
		else
		{
			label_row("Remaining Advance", $exadvance,"","style = 'padding-left: 8px;'");
			hidden('remaining_advance',$exadvance);	
		}
		$exinstallment = $spost['installment'];
		if(empty($exinstallment))
		{
			label_row("Existing Installment", '0.00',"","style = 'padding-left: 8px;'");
		}
		else
		{
			label_row("Existing Installment", $exinstallment,"","style = 'padding-left: 8px;'");	
		...
		row_end();
		text_row_ex(_("New Installment"), 'installment',20,20);
	}
	*/
}
else 
{
	label_row("Installment", "N/A","","style = 'padding-left: 8px;'");	
}
...
$th = array(_("First Name"),_("Last Name"),_("Department"),_("Unit"),_("Time Stamp"),_("Existing Installment"),_("New Installment"));
