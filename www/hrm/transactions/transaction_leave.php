...
$transaction_code1 = EMPLOYEE_PENALTY;
$transaction_code = EMPLOYEE_LEAVE;
//----message controller----
...
function can_process()
{
    ...
    return
            ...
            ( ($_POST[ 'employee_id' ] > 0) && ($_POST[ 'employee_leave_id' ] > 0)) ?
                    ...
                    !is_empty( $_POST[ 'leave_start_date' ], "Leave Start Date" ) &&
                    !is_empty( $_POST[ 'leave_end_date' ], "Leave End Date" ) &&
                    is_number( $_POST[ 'deduct_leave' ], "Deduction Leaves" ) &&
                    is_date( $_POST[ 'leave_start_date' ], "Leave Start Date" ) &&
                    is_date( $_POST[ 'leave_end_date' ], "Leave End Date" ) &&
                    date_sanity_check( $_POST[ 'leave_start_date' ], $_POST[ 'leave_end_date' ], "Leave Start Date", "Leave End Date", 1 ) :
                    0
            )
    ;
}
...
if ( $_POST[ 'employee_id' ] > 0 )
{
    ...
    if ( $_POST[ 'employee_leave_id' ] > 0 )
    {
        ...
        if ( strcmp( $Mode, 'Edit' ) == 0 )
        {
            label_row( "Remaining Leaves", $_POST[ 'remaining_leave' ], "", "style = 'padding-left: 8px;'" );
            hidden( 'remaining_leave', $_POST[ 'remaining_leave' ] );
            text_row_ex( _( "Deduct Leaves" ), 'deduct_leave', 20, 20 );
            date_row( "Leave Start Date", 'leave_start_date' );
            date_row( "Leave End Date", 'leave_end_date' );
        }
        else
        {
...
$th = array( _( "First Name" ), _( "Last Name" ), _( "Department" ), _( "Unit" ), _( "Time Stamp" ), _( "Fiscal Year" ), _( "Leave Start" ), _( "Leave End" ), _( "Leave Type" ), _( "Assigned Leaves" ), _( "Remaining Leaves" ) );
