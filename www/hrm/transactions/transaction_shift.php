...
$page_security = 'SA_HRM_SHIFT_CHANGE_ENTRY';
$path_to_root = "../..";
//----include files------------
...
$transaction_code = EMPLOYEE_SHIFT;
//----message controller----
...
function can_process()
{
	...
	return 
			...
			((strcmp($Mode,'UPDATE_ITEM')==0)?1:!is_empty($_POST['new_shift'], ' New Shift'))  &&
...
if($_POST['employee_id'] > 0)
{
	...
	{
		$shift_db -> select($_POST['transaction_int1'],$dpost);
		$exdesig = $dpost['name'];
		$shift_db -> select($_POST['transaction_int2'],$dpost);
		$newdesig = $dpost['name'];
		label_row("Existing Shift", $exdesig,"","style = 'padding-left: 8px;'");
		label_row("New Shift", $newdesig,"","style = 'padding-left: 8px;'");
	}
	else
	{
		$epost = array();
		$employee_db -> select($_POST['employee_id'],employee_model::PERSONAL,&$epost);
		$shift_db -> select($epost['shift_id'],$dpost);
		$exdesig = $dpost['name'];
		label_row("Existing Shift", $exdesig,"","style = 'padding-left: 8px;'");
		row_start();			
		$arr = array('shift_id' => null,'name' => array('table' => $prefix."shift","label" => "name",'value' => array(array('operator' => '!=' ,'attachment' => 'and','value' => $epost['shift_id'], 'label' => 'shift_id' ))));
		multiple_array_selector('new_shift', _("New Shift"), $shift_db -> search_advanced($arr), 0);
		row_end();
	}
}
else 
{
	label_row("Existing Shift", "N/A","","style = 'padding-left: 8px;'");	
}
...
$th = array(_("First Name"),_("Last Name"),_("Department"),_("Unit"),_("Time Stamp"),_("Old Shift"),_("New Shift"));
