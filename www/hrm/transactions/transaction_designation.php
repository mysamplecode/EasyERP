...
$page_security = 'SA_HRM_DESIGNATION_ENTRY';
$path_to_root = "../..";
//----include files------------
...
$transaction_code = EMPLOYEE_DESIGNATION;
//----message controller----
...
function can_process()
{
	...
	return 
			...
			((strcmp($Mode,'UPDATE_ITEM')==0)?1:!is_empty($_POST['new_designation'], ' New Designation'))  &&
...
{
	$dpost = array();
	if(isset($_POST['transaction_int1']) && isset($_POST['transaction_int2']))
	{
		$designation_db -> select($_POST['transaction_int1'],$dpost);
		$exdesig = $dpost['name'];
		$designation_db -> select($_POST['transaction_int2'],$dpost);
		$newdesig = $dpost['name'];
		label_row("Existing Designation", $exdesig,"","style = 'padding-left: 8px;'");
		label_row("New Designation", $newdesig,"","style = 'padding-left: 8px;'");
	}
	else
	{
		$epost = array();
		$employee_db -> select($_POST['employee_id'],employee_model::PERSONAL,&$epost);
		$designation_db -> select($epost['designation_id'],$dpost);
		$exdesig = $dpost['name'];
		label_row("Existing Designation", $exdesig,"","style = 'padding-left: 8px;'");
		row_start();			
		$arr = array('designation_id' => null,'name' => array('table' => $prefix."designation","label" => "name",'value' => array(array('operator' => '!=' ,'attachment' => 'and','value' => $epost['designation_id'], 'label' => 'designation_id' ))));
		multiple_array_selector('new_designation', _("New Designation"), $designation_db -> search_advanced($arr), 0);
		row_end();
	}
}
else 
{
	label_row("Existing Designation", "N/A","","style = 'padding-left: 8px;'");	
}
...
$th = array(_("First Name"),_("Last Name"),_("Department"),_("Unit"),_("Time Stamp"),_("Old Designation"),_("New Designation"));
