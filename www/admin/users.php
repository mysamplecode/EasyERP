	...
	if (can_process())
	{
    	if ($selected_id != -1) 
    	{
    		update_user_prefs($selected_id,
    			get_post(array('user_id', 'real_name', 'phone', 'email', 'role_id', 'language',
					'print_profile', 'rep_popup' => 0, 'pos','employee_id')));
    	...
    	else 
    	{
    		add_user($_POST['user_id'], $_POST['real_name'], md5($_POST['password']),
				...
				$_POST['print_profile'], check_value('rep_popup'), $_POST['pos'], $_POST['employee_id']);
			$id = db_insert_id();
...
if ($selected_id != -1) 
{
  	if ($Mode == 'Edit') {
		...
		$_POST['employee_id'] = $myrow["employee_id"];
                $employee_tbl = array();
                $flag = $employee_db->select( $_POST['employee_id'], employee_model::PERSONAL, $employee_tbl );
                if($flag)
                {
                    $_POST['unit_id'] = $employee_tbl["unit_id"];
                    $_POST['department_id'] = $employee_tbl["department_id"];
                }
	}
...
if($_POST['employee_id'] > 0)
{
	...
	$employee_db -> select($_POST['employee_id'],employee_model::PERSONAL,$employee_personal_tbl);
