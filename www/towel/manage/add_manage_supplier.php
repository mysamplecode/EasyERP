...
include_once($path_to_root . "/towel/includes/db/supplier_db.inc"); 
...
function can_process()
{
 ...
 global $Mode;
	...
	{
		display_error(_("The  supplier cannot be empty."));
		set_focus('stype');
		return false;
	}
...
if ($Mode=='ADD_ITEM' && can_process())
{
 	...
 	add_tsupplier($_POST['name'],$_POST['address'],@$_POST['ntn'],@$_POST['contact_pname'],$_POST['contact_pdesig'],@$_POST['contact_pno'],@$_POST['stype']);
...
if ($Mode=='UPDATE_ITEM' && can_process() )
{
	...
	update_tsupplier($selected_id,$_POST['name'], $_POST['address'],@$_POST['ntn'],@$_POST['contact_pname'],$_POST['contact_pdesig'],@$_POST['contact_pno'],@$_POST['stype']);
	display_notification(_('Selected supplier has been updated'));
	$Mode = 'RESET';
}
if ($Mode == 'Delete')
{
...
$rep_file = find_custom_file("/towel/reports/rep_supplier.php");
if ($rep_file) {
	require($rep_file);
}
...
page(_($help_context = "Towel  Suppliers"), false, false, "", ''); 
...
br();
display_heading("Add new supplier type"); br();
...
text_row_ex(_("Contact Person Number").':', 'contact_pno',50,200);  
 custom_select_row("Supplier Type","stype",array("Commercial Dyer"=>"Commercial Dyer","Greige Manufacturer"=>"Greige Manufacturer"));
end_table(1);
...
row_end();
row_start();
check_cells("Remove supplier type", "remove_stype", "all");
 row_end();
row_start();
...
end_table(1);
	hidden('REP_ID', 'tsupplier' );
	 ...
	 br();
	br(); 
 display_heading ("Filter Suppliers"); br();
start_table(TABLESTYLE2);
row_start();
...
end_table(1);
 $th = array (_('Name'),_('Address'),_('NTN'),_('Contact Person Name'),_('Contact Person Designation'),_('Contact Person Number'),_('Supplier Type') );
